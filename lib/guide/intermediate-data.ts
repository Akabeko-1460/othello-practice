...
export const INTERMEDIATE_CONCEPTS: GuideConcept[] = [
  ...
  // ──────────────────────────────────────────────
  {
    ...
    importance: 'high',
    number: 1,
    summary: '辺にあえてC打ちし、相手が隅を取ると辺全体が返る罠。種石を仕込んでおくのがポイント。',
    scenarios: [
      {
        title: 'ストナーの仕組み',
        ...
        steps: [
          {
            comment: '上辺に注目。白がc1〜f1に並び、黒はg1に種石があります。ここからストナーを仕掛けます。',
            tag: 'info',
          ...
          },
          {
            ...
            move: { row: 0, col: 1 },
            player: BLACK,
            comment: '黒b1（C打ち）！通常C打ちは悪手ですが、g1の種石まで白4枚を一気に返しました。これがストナーです！',
            tag: 'good',
            highlights: [1, 2, 3, 4, 5, 6],
          },
          {
            comment: '上辺がほぼ全て黒の確定石候補になりました。種石g1が効いて、C打ちが大量返しの起点になったのです。ストナーは種石の準備がカギです。',
            tag: 'info',
            highlights: [1, 2, 3, 4, 5, 6],
          },
        ],
  ...
  // ──────────────────────────────────────────────
  {
    ...
    importance: 'high',
    number: 2,
    summary: '終盤の空き領域が偶数個のとき、後から打つ方が有利。白番は自然と恩恵を受けやすい。',
    scenarios: [
      {
        title: '偶数理論の基本',
        ...
        steps: [
          ...
          },
          {
            ...
            comment: '偶数個の空きでは、先に打った方（黒）が必ず1手損します。なぜなら最後の1手を相手（白）が打てるからです。',
            tag: 'warning',
          },
          {
            comment: '白番（後手）は自然と偶数理論の恩恵を受けます。黒番は空き領域を奇数にする工夫（奇数理論）で対抗しましょう。',
            tag: 'info',
          },
        ],
      },
  ...
  // ──────────────────────────────────────────────
  {
    ...
    importance: 'medium',
    number: 3,
    summary: '先手（黒）が偶数理論を破るために、奇数個の空き領域を作って最後の手を奪う戦略。',
    scenarios: [
      {
        title: '奇数理論で先手有利に',
        ...
        steps: [
          {
            comment: '空きマスが3つ（奇数）あります。奇数個の空き領域では先手（黒）が最後の手を打てます！',
            tag: 'info',
            highlights: [55, 62, 63],
          },
          {
            comment: '奇数理論: 空きが奇数なら先手が最終手。偶数なら後手が最終手。終盤は空き領域の偶奇を意識して打ちましょう。',
            tag: 'good',
          },
        ],
      },
  ...
  // ──────────────────────────────────────────────
  {
    ...
    importance: 'high',
    number: 4,
    summary: '後の手筋のために事前に配置しておく石。ストナーの仕掛けや終盤の大量返しの布石。',
    scenarios: [
      {
        title: '種石を仕込む',
        ...
        steps: [
          ...
          },
          {
            ...
            move: { row: 0, col: 5 },
            player: BLACK,
            comment: '黒f1に種石！今は孤立して弱く見えますが、これが後のストナーの軸になります。',
            tag: 'good',
            highlights: [5],
          },
          {
  ...
  // ──────────────────────────────────────────────
  {
    ...
    importance: 'medium',
    number: 5,
    summary: '各マスの戦略的価値を数値化した評価表。隅が最高(100)、X打ちが最低(-50)。',
    scenarios: [
      {
        title: '位置の価値を知る',
        ...
        steps: [
          {
            comment: '盤面の四隅（緑ハイライト）は最も価値が高いマスです（100点）。取れば確定石になります。',
            tag: 'good',
            highlights: [0, 7, 56, 63],
          },
          {
            comment: '隅の斜め隣のX打ち（赤ハイライト）は最も危険なマス（-50点）。打つと隅を献上する可能性大。',
            tag: 'bad',
            highlights: [9, 14, 49, 54],
          },
          {
            comment: '隅の隣のC打ち（黄色ハイライト）もリスクが高いマス（-20点）。理由なく打つのは避けましょう。',
            tag: 'warning',
            highlights: [1, 6, 8, 15, 48, 55, 57, 62],
          },
          {
            comment: '辺のそれ以外のマス（青ハイライト）は中程度の価値（5〜10点）。盤面状況で変化するので、TOFは目安として覚えておきましょう。',
            tag: 'info',
            highlights: [2, 3, 4, 5, 16, 24, 32, 40, 23, 31, 39, 47, 58, 59, 60, 61],
          },
        ],
      },
    ],
